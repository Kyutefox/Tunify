    ...
    for (final c in clients) {
      try {
        ...
        final clientContext = <String, dynamic>{
          ...
          if (c['androidSdkVersion'] != null) 'androidSdkVersion': c['androidSdkVersion'],
        };
        ...
        audioFormats.sort(
          (a, b) => ((a['bitrate'] as int?) ?? 0).compareTo((b['bitrate'] as int?) ?? 0),
        );
        ...
        if (preferAac) {
          final aac = audioFormats.where((f) {
            ...
            return mime.contains('mp4') || mime.contains('m4a') || mime.contains('aac');
          }).toList();
        ...
        final mimeType = ((best['mimeType'] as String?) ?? 'audio/mp4').split(';').first;
        ...
        return YouTubeStream(
          ...
          url: url,
          quality: kbps >= 160 ? 'high' : kbps >= 80 ? 'medium' : 'low',
