...
class SupabaseCreateController {
  ...
  Future<void> insertPlaylistTracks(String userId, String playlistId, List<Map<String, dynamic>> rows) async {
    if (rows.isEmpty) return;
    final mapped = <Map<String, dynamic>>[];
    for (var i = 0; i < rows.length; i++) {
      mapped.add(SupabaseMappers.songMapToTrackRow(userId, playlistId, i, rows[i]));
    }
  ...
  Future<void> insertFolderPlaylists(String userId, String folderId, List<String> playlistIds) async {
  ...
  Future<void> insertLikedSongs(String userId, List<Map<String, dynamic>> likedSongs) async {
  ...
  Future<void> insertRecentlyPlayed(String userId, List<Map<String, dynamic>> songs) async {
    if (songs.isEmpty) return;
    final rows = songs.take(50).map((s) => {
          'user_id': userId,
          'song_id': s['id'],
  ...
  Future<void> insertRecentSearches(String userId, List<String> queries) async {
    ...
    final rows = queries.take(20).map((q) => {'user_id': userId, 'query': q, 'searched_at': now}).toList();
  ...
  Future<void> insertDownloadedSongIds(String userId, List<String> ids) async {
    ...
    final now = DateTime.now().toUtc().toIso8601String();
    final rows = ids.map((id) => {'user_id': userId, 'song_id': id, 'added_at': now}).toList();
  ...
  Future<void> insertFollowedArtists(String userId, List<Map<String, dynamic>> artists) async {
    ...
    final rows = artists.map((a) => {
          ...
          'followed_at': a['followedAt'] ?? DateTime.now().toUtc().toIso8601String(),
        }).toList();
  ...
  Future<void> insertFollowedAlbums(String userId, List<Map<String, dynamic>> albums) async {
