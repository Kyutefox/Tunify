...
class SupabaseGetController {
  ...
  Future<Map<String, dynamic>?> fetchLibraryData(String userId) async {
    try {
      final playlistsRows = await _client.from(StorageKeys.supabaseUserPlaylists).select().eq('user_id', userId);
      final tracksRows = await _client
          ...
          .order('playlist_id')
          .order('position');
      final folderRows = await _client.from(StorageKeys.supabaseUserFolders).select().eq('user_id', userId);
      final junctionRows = await _client.from(StorageKeys.supabaseUserFolderPlaylists).select().eq('user_id', userId);
      final settingsRow = await _client.from(StorageKeys.supabaseUserLibrary).select().eq('user_id', userId).maybeSingle();
      final likedRows = await _client.from(StorageKeys.supabaseUserLikedSongs).select().eq('user_id', userId).order('position');
      final shuffleRows = await _client.from(StorageKeys.supabaseUserPlaylistShuffle).select().eq('user_id', userId);
      final followedArtistRows = await _client.from(StorageKeys.supabaseUserFollowedArtists).select().eq('user_id', userId).order('followed_at');
      final followedAlbumRows = await _client.from(StorageKeys.supabaseUserFollowedAlbums).select().eq('user_id', userId).order('followed_at');
      ...
      for (final r in junctionRows as List) {
        ...
        if (fid != null && pid != null) folderIdToPlaylistIds.putIfAbsent(fid, () => []).add(pid);
      }
      ...
      for (final r in tracksRows as List) {
        final map = Map<String, dynamic>.from(r as Map);
        final pid = map['playlist_id'] as String?;
        if (pid != null) playlistIdToTracks.putIfAbsent(pid, () => []).add(SupabaseMappers.trackRowToSongMap(map));
      }
      ...
      for (final r in shuffleRows as List) {
        final map = Map<String, dynamic>.from(r as Map);
        final pid = map['playlist_id'] as String?;
        if (pid != null) shuffleMap[pid] = map['shuffle_enabled'] as bool? ?? false;
      }
      ...
      final likedSongs = (likedRows as List).map((r) => SupabaseMappers.trackRowToSongMap(Map<String, dynamic>.from(r as Map))).toList();
      final playlistShuffles = <String, dynamic>{};
      for (final p in playlists) {
        if (p['shuffleEnabled'] == true) playlistShuffles[p['id'] as String] = true;
      }
  ...
  Future<({
    ...
    final existingPlaylists = await _client.from(StorageKeys.supabaseUserPlaylists).select('id, updated_at').eq('user_id', userId);
    final remotePlaylistUpdatedAt = <String, String>{};
    for (final r in existingPlaylists as List<dynamic>) {
      final m = r as Map;
      final id = m['id'] as String?;
      if (id != null) remotePlaylistUpdatedAt[id] = m['updated_at']?.toString() ?? '';
    }

    final existingFolders = await _client.from(StorageKeys.supabaseUserFolders).select('id, name').eq('user_id', userId);
    final junctionRows = await _client.from(StorageKeys.supabaseUserFolderPlaylists).select('folder_id, playlist_id').eq('user_id', userId);
    ...
    for (final r in junctionRows as List<dynamic>) {
      ...
      if (fid != null && pid != null) (remoteFolderPlaylistIds.putIfAbsent(fid, () => [])).add(pid);
    }
    final remoteFolderName = <String, String>{};
    for (final r in existingFolders as List<dynamic>) {
      final m = r as Map;
      final id = m['id'] as String?;
      if (id != null) remoteFolderName[id] = m['name']?.toString() ?? '';
    }

    return (playlistUpdatedAt: remotePlaylistUpdatedAt, folderName: remoteFolderName, folderPlaylistIds: remoteFolderPlaylistIds);
  }
  ...
  Future<Map<String, dynamic>?> fetchPlaybackSettings(String userId) async {
    try {
      final res = await _client.from(StorageKeys.supabaseUserPlaybackSettings).select().eq('user_id', userId).maybeSingle();
      if (res == null) return null;
      return {
        PlaybackSettingKeys.volumeNormalization: res['volume_normalization'] as bool? ?? false,
        PlaybackSettingKeys.showExplicitContent: res['show_explicit_content'] as bool? ?? true,
        PlaybackSettingKeys.smartRecommendationShuffle: res['smart_recommendation_shuffle'] as bool? ?? true,
        PlaybackSettingKeys.crossfadeDurationSeconds: res['crossfade_duration_seconds'] as int? ?? 0,
        PlaybackSettingKeys.gaplessPlayback: res['gapless_playback'] as bool? ?? true,
      };
    } catch (_) {
      return null;
    }
  ...
  Future<List<String>?> fetchRecentSearches(String userId) async {
    try {
      ...
      return list.map((r) => (r as Map)['query']?.toString()).where((s) => s != null && s.trim().isNotEmpty).cast<String>().toList();
    } catch (_) {
      return null;
    }
  }
  ...
  Future<List<String>?> fetchDownloadedSongIds(String userId) async {
    try {
      final rows = await _client.from(StorageKeys.supabaseUserDownloadedSongs).select('song_id').eq('user_id', userId).order('added_at', ascending: false);
      final list = rows as List<dynamic>?;
      if (list == null || list.isEmpty) return null;
      return list.map((r) => (r as Map)['song_id']?.toString()).where((s) => s != null && s.isNotEmpty).cast<String>().toList();
    } catch (_) {
      return null;
    }
  }
  ...
  Future<Map<String, dynamic>?> fetchYtPersonalization(String userId) async {
    try {
      final res = await _client.from(StorageKeys.supabaseYtPersonalization).select('visitor_data, api_key, client_version').eq('user_id', userId).maybeSingle();
      if (res == null) return null;
      return {
